...
# -- JSON serialisation (BOM-free, clean output) --------------------------------
function ConvertTo-CleanJson($obj) {
    $json = $obj | ConvertTo-Json -Depth 10
    # Strip leading BOM (U+FEFF) and remove zero-width / invisible characters
    # that can silently corrupt JSON parsing in downstream tools.
    $json = $json.TrimStart([char]0xFEFF)
    foreach ($ch in @([char]0x200B, [char]0x200C, [char]0x200D, [char]0xFFFE, [char]0xFFFD)) {
        $json = $json.Replace([string]$ch, '')
    }
    return $json
}
    ...
    foreach ($root in $roots) {
        ...
        foreach ($ext in $SCAN_EXTENSIONS) {
            Get-ChildItem -Path $root -Recurse -Filter $ext | ForEach-Object {
                ...
                if ($rel -clike 'mods/*') { $rel = 'Mods' + $rel.Substring(4) }
                if ($rel -clike 'maps/*') { $rel = 'Maps' + $rel.Substring(4) }
                $size    = $_.Length
...
if ($buildPublic) {
    ...
    if (Prompt-YN "Write update-manifest.json?" $true) {
        ...
        [System.IO.File]::WriteAllText($pubPath, (ConvertTo-CleanJson $pubManifest))
...
if ($buildBeta) {
    ...
    if (Prompt-YN "Write beta-manifest.json?" $true) {
        ...
        [System.IO.File]::WriteAllText($betaPath, (ConvertTo-CleanJson $betaManifest))
